...
```
* Так же мы можем посмотреть список всех коммитов с отображеннием всех веток графически с помощью команды:
```
...
* После внесения изменения в новую ветку (черновик, тест) мы можем перенести все данные и изменения в основную или любую другую ветку. Для этого предворительно сохранив и создав новый коммит переходим в ту ветку куда хотим слить (соединить) текущие изменения с помощью команды:
...
Если все впорядке можно произвести слияние веток с помощью команды:
```
...
```
* В случае если слияние небыло произведенно то Git недаст удалить такую ветку и ошибка будет выглядет так:
